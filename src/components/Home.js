...
// import { getLoginUser,updateLoginUser } from "../store/models/user";
...
class Home extends Component {
  constructor(props) {
    ...
    // console.log("getting user home ", getLoginUser());
  }

  componentDidMount() {
    axios
      .get("http://backend.dev.everrecruit.indusvalleylabs.com/test/api")
      .then(response => {
        this.setState({
  ...
  add_to_cart(product, index) {
    ...
    cart_products = products.filter(function(product) {
      return product.hasOwnProperty("quantity") && product.quantity > 0
        ? product
        : null;
    });
